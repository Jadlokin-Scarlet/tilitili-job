...
@Slf4j
@Component
public class SpiderAllVideoViewJob {
    private final TaskManager taskManager;
    private final TouhouAllMapper touhouALlMapper;
    private final RedisCache redisCache;

    @Autowired
    public SpiderAllVideoViewJob(TaskManager taskManager, TouhouAllMapper touhouALlMapper, RedisCache redisCache) {
    ...
    protected void executeInternal() {
        Integer start = (Integer) redisCache.get(RedisKeyEnum.SPIDER_ALL_VIDEO_REDIS_KEY.getKey());
        int count = touhouALlMapper.count(new BaseQuery<>());
        if (start == null || start > count) {
            start = 0;
        }
        BatchTask batchTask = new BatchTask().setType(AutoBatchSpiderVideo.value).setReason(RE_SPIDER_All_VIDEO.value);
        ...
        redisCache.set(RedisKeyEnum.SPIDER_ALL_VIDEO_REDIS_KEY.getKey(), start);
    }
